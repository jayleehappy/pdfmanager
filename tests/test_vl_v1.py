...
print(f"[INFO] UmiOCR 检测: {len(gt_items)} 个区域")
...
for p in parsing:
    ...
    try:
        ...
        print(f"  [{label}] (显示失败: {e})")

# 保存完整结果
out = {
    "model": "PaddleOCR-VL v1",
    "device": "cpu",
...
with open(OUTPUT_DIR / "vl_v1_result.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False, indent=2)

print(f"\n[OK] 结果已保存: {OUTPUT_DIR / 'vl_v1_result.json'}")
print("完成!")
